...
| `docs`         | 3          | Documentation fetch (`fetch_document_links`, `fetch_documentation_page`, `list_documentation_sources_tool`) |
| `scrapling`    | 2          | `s_fetch_page`, `s_fetch_pattern` (web fetch modes)                                                        |
| `doc_catalog_manager` | 1          | Catalog admin helpers (already wrapped by the `documentation_catalog` aggregate)                           |
...
5. **`doc_fetch_suite`** – Documentation fetch helpers (links, full pages, catalog listing). Complements the `documentation_catalog` aggregate without surfacing three near-identical fetchers.
...
8. **Existing** **`documentation_catalog`** – Left untouched; still governs catalog administration.
...
- `documentation_catalog.manage` (catalog manager) remains hidden behind the aggregate entry.
...
- `facade.search` is hidden; we no longer need the placeholder verification hook now that the documentation fetch stack/Scrapling provide canonical fetch/search.
...
- Consider pulling documentation surfacing into Strata if we introduce more advanced routing, so there’s a single “knowledge ops” entry.
