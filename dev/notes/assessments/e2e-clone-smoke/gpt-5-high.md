...
- Core vs optional: core templates declare only self-management servers (`custom`, `integrator`, `one_mcp`, `public_mcp_catalog`, `tool_aggregator`). The optional starter bundle lives in `bundles/starter/` and installs by copying that folder into `${STELAE_CONFIG_HOME}/bundles/`.
