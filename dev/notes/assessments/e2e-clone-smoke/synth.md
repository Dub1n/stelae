...
- [x] **Preread:** `README.md` (Core vs Optional Stack, Declarative Tool Aggregations), `docs/ARCHITECTURE.md` (Config overlays), the five overlapping task docs from Active Risks #3, plus `docs/e2e_clone_smoke_test.md` (Manual stages). _Re-read everything while drafting the new consolidated workbook so references and terminology match._
- [x] Merge the existing task docs into `dev/tasks/stelae-smoke-readiness.md` with three sections: (a) Catalog hygiene, (b) Harness + restart reliability, (c) Codex orchestration. Keep dated appendices for historical run logs; link this doc from `dev/progress.md` and `docs/e2e_clone_smoke_test.md`. _New workbook added with the three streams, consolidated checklists, and appendices that host the prior run logs; all five legacy task docs now redirect there and `dev/progress.md` links to the new anchors._
- [x] Expand README and ARCHITECTURE with an “Overlay workflow & guardrails” subsection that spells out the renderer/pytest loop and explicitly points to the starter bundle installer for aggregates. _Both docs now include the prescribed loop (`process_tool_aggregations.py --scope default`, `--scope local`, `make render-proxy`, `pytest tests/test_repo_sanitized.py`, optional `make verify-clean`) and reiterate that overlays live in `${STELAE_CONFIG_HOME}` while runtime artifacts live in `${STELAE_STATE_HOME}`._
- [x] Update `docs/e2e_clone_smoke_test.md` to emphasize that long pauses at “Installing starter bundle…” or “Restarting stack…” indicate orchestration failures, not intended waits. _Guide now documents `--restart-timeout`, `--restart-retries`, `--heartbeat-timeout`, the automatic port-preflight/kill behavior, pm2 log tail collection, and links directly to the new `stelae-smoke-readiness` workbook._
