...
- Remaining Phase 3: complete bundle/installer refactor to write catalog fragments directly to `${STELAE_CONFIG_HOME}/bundles/<name>/catalog.json` with no overlay/.local handling; sanity-check restart/bundle flows to ensure no `.local` files are recreated; align any lingering overlay helpers.
- Remaining Phase 4: full doc sweep for lingering `.local`/tracked-config references; optional render/restart/bundle smoke; run `make render-proxy` / `make verify-clean` after final changes.
