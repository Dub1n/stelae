...
- Completed Phase 3: bundle installer writes catalog fragments directly to `${STELAE_CONFIG_HOME}/bundles/<name>/catalog.json` (no overlay/.local handling); restart/render flow validated via `make render-proxy`.
- Completed Phase 4 (core): doc sweep for `.local`/tracked-config references in README/AGENTS/ARCHITECTURE; targeted tests/passes (`pytest tests/test_install_stelae_bundle.py tests/test_setup_env.py tests/test_repo_sanitized.py` with `PYTHONPATH=.`); render-proxy rerun successfully. Remaining optional: full smoke (`make verify-clean`) if desired.

Open follow-up: refactor tests for strict path validation
- Issue: Full `pytest` now fails because stricter `require_home_path` enforces config/state-home locations. Fixtures still write overrides/discovery/custom_tools under ad-hoc repo tmp paths, causing `ValueError` in custom_tools_server and integrator tests; render proxy tests also assume repo-local paths.
- Plan: Update test fixtures to place tool overrides, discovery caches, custom tools configs, and templates under the provided `${STELAE_CONFIG_HOME}`/`${STELAE_STATE_HOME}` (set via monkeypatch) instead of repo-relative tmp paths. Only relax `require_home_path` guards if absolutely necessary and preferably via test-only hooks.
- Affected tests (observed failures): `tests/test_custom_tools.py::test_env_override_respected`, multiple in `tests/test_stelae_integrator.py` (discovery/overrides/overlay expectations), `tests/test_render_proxy_config.py`, and downstream cases expecting repo-local overlays.
