...
- Completed Phase 4 (core): doc sweep for `.local`/tracked-config references in README/AGENTS/ARCHITECTURE; targeted tests/passes (`pytest tests/test_install_stelae_bundle.py tests/test_setup_env.py tests/test_repo_sanitized.py` with `PYTHONPATH=.`); render-proxy rerun successfully.
- Closed follow-up: fixtures now respect `require_home_path` by writing overlays/discovery/custom-tool configs into `${STELAE_CONFIG_HOME}`/`${STELAE_STATE_HOME}`. Baseline regression `PYTHONPATH=. .venv/bin/pytest --ignore tests/test_e2e_clone_smoke.py` passes clean (skipping only the Codex wrapper smoke variants). No outstanding work remains; keep `make verify-clean` as the optional smoke when PM2/cloudflared are available.
