...
- `scripts/restart_stelae.sh` now runs `scripts/process_tool_aggregations.py --persist-overlay` so hideTool directives are written to the overlay before exporting the runtime JSON. After rerender/restart, the HTTP catalog only exposes the aggregate entries (e.g., `workspace_fs_read`, `workspace_fs_write`, `workspace_shell_control`, `doc_fetch_suite`, `scrapling_fetch_suite`, `memory_suite`, `manage_docy_sources`, `manage_stelae`, `batch`, etc.), fixing the “full raw tool list” regression.
...
3. **Schema cleanup follow-up.** The aggregation visibility/dedupe work now lives in `dev/tasks/stelae-tool-aggregation-visibility.md`; close that task before reattempting the Codex catalog runs here.
