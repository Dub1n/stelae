...
| --- | --- | --- | --- | --- |
| Config-home catalogs & bundles | `${STELAE_CONFIG_HOME}/catalog/*.json` and `${STELAE_CONFIG_HOME}/bundles/*/catalog.json` | Operators/bundle authors | `process_tool_aggregations.py` | Sole editable inputs post-untracked-config cleanup; renderer already uses these to build intended catalog. |
| Tool overrides overlay | `${STELAE_CONFIG_HOME}/tool_overrides.json` | Operators/integrator | render scripts | Defaults embedded in code; deleting resets to embedded defaults. Guarded by `require_home_path` so it must live under config/state home. |
...
| Discovery cache (runtime) | `${STELAE_STATE_HOME}/discovered_servers.json` | 1mcp agent / `manage_stelae discover_servers` | Integrator, operators | Defaults embedded; fresh clones start empty; deleting resets discovery. Must live under config/state home. |
...
5. **Diff helper + retention scripts** – implement `scripts/diff_catalog_snapshots.py` + rotation utilities invoked from `make render-proxy` and restart scripts. ✅ done (diff helper wired into restart/make; retention handled via proxy rotation; add extra pruning later if needed).
...
8. **Post-flip override cleanup** – once `STELAE_USE_INTENDED_CATALOG=1` is the default, stop auto-populating aggregate-derived schemas into `tool_overrides.json`; keep `tool_overrides.json` in `${STELAE_CONFIG_HOME}` (and mirrored in state) solely as a human-edit surface for manual overrides, while aggregates live only in catalog fragments → `intended_catalog.json`. Ensure proxy/restart scripts read `intended_catalog.json` as the canonical source and treat overrides as optional/manual input.
...
- Progress: proxy now emits rotated live snapshots and diagnostics; renderer + restart flow run diff reporting; CLI diff helper exists; verify-clean enforces drift unless `STELAE_ALLOW_LIVE_DRIFT` is set; catalog metrics emitted to `${STELAE_STATE_HOME}`. `STELAE_USE_INTENDED_CATALOG=1` is the default, `scripts/run_restart_stelae.sh` exposes `--intended-catalog`/`--legacy-catalog`, and the smoke harness (`--catalog-mode both`) exercises both code paths in CI. Next focus: telemetry export/metrics consumers and tightening catalog adoption alerts.
