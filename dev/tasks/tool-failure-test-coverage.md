...
- Keep this doc open until the schema-aware tests execute cleanly and the remaining aggregate coverage additions land. The future documentation catalog aggregate will land in the starter bundle overlays; tests load descriptors from `config/bundles/starter_bundle.json` so a bare clone without the optional bundle still exercises the schemas that remain.
