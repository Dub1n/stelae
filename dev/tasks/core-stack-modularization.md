...
- [x] Trim tracked templates/manifests so only the core servers ship out of the box; move the documentation catalog, tool aggregator, Memory, Strata, etc. into an optional bundle.
...
- [x] Relocate documentation catalog manager aggregation + non-core tool overrides into `${STELAE_CONFIG_HOME}` overlays when the starter bundle is applied; ensure existing users retain their local overlays.
...
- Dropping the legacy documentation MCP (and other optional servers) from the tracked config should include an alternative for surfacing repo docs, or instructions for enabling the replacement via the starter script.
