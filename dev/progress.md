...
- [ ] [user-config-overlays](./tasks/user-config-overlays.md) Split repo-shipped defaults from per-instance overlays (e.g. `~/.config/stelae`) so clones stay generic while local MCP customisations auto-load outside git.
...
- [ ] [tool-aggregations](./tasks/tool-aggregations.md) Add declarative tool aggregation helper + config to expose composite tools while hiding noisy downstream entries.
