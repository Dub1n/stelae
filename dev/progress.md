...
- [ ] [notice-delivery](./tasks/notices.md) Deliver a one-shot MCP notice queue so bootstrap helpers (setup_env, bundle installs, restart automation) can surface "we recreated X" summaries to users without spamming responses.
- [ ] [portable-bundles](./tasks/portable-bundles.md) Deliver drop-in bundle folders (catalog + install metadata + helper scripts) so optional servers/tools install via copied folders in `${STELAE_CONFIG_HOME}/bundles/` without overlay duplication.
- [ ] [strata-integration](./tasks/strata-integration.md) Build the Strata reconciler flow (`make promote … TARGET=strata` / `manage_stelae run_reconciler`) so rare capabilities route through Strata actions instead of bloating the core proxy manifest.
- [~] [stelae-mcp-catalog-consistency](./tasks/stelae-smoke-readiness.md#catalog-hygiene) Ensure the Stelae MCP proxy consistently advertises the curated tool suite and that the scripted `codex exec --json --full-auto` harness can call those tools without manual fallbacks. *(2025-11-12: fresh CLI run recorded in `logs/codex-catalog-smoke-latest.jsonl` shows `workspace_fs_read` succeeding again; we still need to wire the remaining tool-call assertions into the harness.)*
