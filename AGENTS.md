...
- Treat the README “Stack Snapshot” table as the live roster for MCP services (proxy, aggregator, integrator, custom tools, FastMCP bridge, discovery agent, etc.); each entry lists the transport, launch command, and purpose so you can immediately verify why a server is missing from `pm2 status`. Use `DEVELOPMENT.md` for the extended per-server procedures.
...
- Environment: run `python scripts/setup_env.py`, edit `${STELAE_ENV_FILE}` (defaults to `${STELAE_CONFIG_HOME}/.env`) to update path/binary variables, then run `make render-proxy`. The helper now auto-seeds catalog/override stubs inside `${STELAE_CONFIG_HOME}`/`${STELAE_STATE_HOME}` and relocates stray JSON files if paths drift, so fresh installs and deleted overlays recover automatically. Keep `${STELAE_ENV_FILE}` local; renderers inject values for PM2.
- Core stack = mcp-proxy, custom tools, the Stelae integrator, the tool aggregator helper, the 1mcp stdio agent, the public 1mcp catalog bridge, and the FastMCP bridge. The starter bundle (Basic Memory, Strata, Fetch, Scrapling, filesystem/ripgrep/terminal helpers) ships as the folder `bundles/starter/`; install or update it via `python scripts/install_stelae_bundle.py [--server name...]` so the folder copies into `${STELAE_CONFIG_HOME}/bundles/` and registers install refs without touching tracked overlays. The Codex MCP wrapper is intentionally excluded from this bundle—build the release via `~/dev/codex-mcp-wrapper/scripts/build_release.py`, copy it into `${STELAE_CONFIG_HOME}/codex-mcp-wrapper/releases/<version>`, and then run the manual `manage_stelae install_server` flow documented in `DEVELOPMENT.md` when you explicitly want the wrapper.
- Overlay workflow (docs/ARCHITECTURE.md + DEVELOPMENT.md): aggregation/override data lives under `${STELAE_CONFIG_HOME}` (user overlays plus optional bundle fragments, no tracked templates). Run `python scripts/process_tool_aggregations.py --scope local` to emit `${TOOL_OVERRIDES_PATH}` and `${INTENDED_CATALOG_PATH}`, then `make render-proxy` and `pytest tests/test_repo_sanitized.py`; wrap up with `make verify-clean` (or `./scripts/verify_clean_repo.sh --skip-restart`) to confirm render/restart automation keeps `git status` empty. The renderer now prefers `${STELAE_STATE_HOME}/live_descriptors.json`, fails unless `--allow-stale-descriptors` is passed when that snapshot is missing/stale, appends drift summaries to `${STELAE_STATE_HOME}/live_catalog_drift.log`, and `--verify` fails when the live catalog snapshot is absent unless drift is explicitly allowed. Restart flow also prints a best-effort diff via `scripts/diff_catalog_snapshots.py` (fail-on-drift unless `STELAE_ALLOW_LIVE_DRIFT` is set), emits catalog metrics, and prunes history. The catalog layering is consolidated into `intended_catalog.json` / `live_catalog.json`; `STELAE_USE_INTENDED_CATALOG=1` now ships in `.env.example`, so only override it (set to `0` **or** run `scripts/run_restart_stelae.sh --legacy-catalog`) when you intentionally need the runtime fallback. The tool-aggregator server now prefers the intended catalog as its runtime source; `STELAE_TOOL_AGGREGATIONS` remains an explicit override path.
- Path guard + regression: `require_home_path` now rejects mutable paths outside `${STELAE_CONFIG_HOME}`/`${STELAE_STATE_HOME}`; fixtures/helpers assume those roots. Baseline check `PYTHONPATH=. .venv/bin/pytest --ignore tests/test_e2e_clone_smoke.py` should pass with only the expected skips before shipping config changes.
...
- Smoke/diagnostic helpers: use `dev/debug/check_connector.py`, `dev/debug/chatgpt_connector_probe.py`, and the SSE harness examples in `DEVELOPMENT.md` to verify manifests/search/fetch behavior.
...
- User-editable overlays live under `${STELAE_CONFIG_HOME}` (default `~/.config/stelae`). Automation writes runtime artifacts (`proxy.json`, merged tool overrides, tool schema status, intended/live catalogs, etc.) to `${STELAE_STATE_HOME}` so git never sees machine-specific data; route any new generated files there as well. Run `pytest tests/test_repo_sanitized.py` before committing template changes to ensure tracked configs stay placeholder-only.
- Two-layer overlays from README/DEVELOPMENT: `${STELAE_CONFIG_HOME}/.env` is the editable copy, `${STELAE_CONFIG_HOME}/.env.local` holds hydrated secrets/defaults. Mutable JSON (overrides, catalog fragments under `catalog/`, bundle data under `bundles/`, discovery, custom tools, schema status) now lives directly under `${STELAE_CONFIG_HOME}` (no `.local` suffix) and runtime artifacts under `${STELAE_STATE_HOME}` (`${PROXY_CONFIG}`, `${TOOL_OVERRIDES_PATH}`, `${TOOL_SCHEMA_STATUS_PATH}`, `${INTENDED_CATALOG_PATH}`, `${STELAE_STATE_HOME}/live_catalog.json`, etc.).
...
- Keep `${STELAE_ENV_FILE}` out of git; renderers (`scripts/render_proxy_config.py`, `scripts/render_cloudflared_config.py`) handle substitution. Regenerate Cloudflare configs via `make render-cloudflared`, store credentials under `~/.cloudflared`, and validate the public endpoint with the curl/JQ commands in README/DEVELOPMENT.
...
- Validate readiness with the README/DEVELOPMENT commands: `curl -s http://localhost:${PROXY_PORT:-9090}/.well-known/mcp/manifest.json | jq '{servers, tools: (.tools | map(.name))}'`, the Cloudflare equivalent (`https://mcp.infotopology.xyz`), and `curl -skI https://mcp.infotopology.xyz/stream` for SSE reachability. Missing catalog entries usually mean the aggregator or FastMCP bridge is offline.
- Keep Cloudflare managed by PM2 (`pm2 start "cloudflared tunnel run stelae" --name cloudflared`) and restart both `mcp-proxy` and `cloudflared` after updating `.env` or proxy configs. Follow the README/DEVELOPMENT snippet to confirm the DNS route (`cloudflared tunnel route dns stelae mcp.infotopology.xyz`) before exposing the stack publicly.
- The SSE harness and example Python snippet in DEVELOPMENT.md exercise `/rg/sse` and `/fetch/sse`; use them (or `dev/debug/chatgpt_connector_probe.py`) whenever you need to debug streamable endpoints or verify the FastMCP bridge still exposes the full catalog.
...
- The watchdog `scripts/watch_public_mcp.py` shares the PM2 ensure logic described in README/DEVELOPMENT—lean on it (or replicate its steps) when monitoring the tunnel/proxy from CI machines.
...
- Aggregations can also pin a specific downstream server via `downstreamServer`. The stdio helper forwards that hint as `serverName` in every proxy `tools/call`, so catalog cleanups (renames, hidden entries) never break composites like `workspace_fs_read` (`fs`) or `workspace_shell_control` (`sh`). Keep the bundle/catalog metadata accurate so the proxy doesn’t fall back to whichever server happens to own the tool name.
...
- If new generators or discovery runs create files, write them to `${STELAE_STATE_HOME}` (runtime artifacts) or `${STELAE_CONFIG_HOME}` (human-edited overlays) and never directly to tracked templates; deleting the config-home copy plus rerendering should reset to embedded defaults. The intended/live catalog proposal means aggregated descriptors may soon be sourced from `intended_catalog.json` rather than the current runtime overrides—flag any assumptions about catalog persistence when modifying these flows.
