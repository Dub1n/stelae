...
- Fidelity vs. load: the harness avoids extra restarts/builds (skip pm2 kill/port kill, reuse proxy binary) purely to reduce system stress. We intentionally do **not** add a “force rebuild” flag to the harness so the flow stays close to real usage while keeping load bounded; only deviations necessary to make the run feasible/safe are included.
