...
- Use `--reuse-workspace` when you want to keep an existing sandbox warm between runs, and `--cleanup-only [--workspace /path]` to retroactively delete kept workspaces without provisioning a new clone. `--force-workspace` overwrites a directory even when the marker file is missing so deterministic paths remain available for log collection.
...
**Verification loop.** The clone-smoke harness itself is the only orchestrator now: after every catalog or restart change, rerun `scripts/run_e2e_clone_smoke_test.py` (or the relevant stage) so it provisions the sandbox, re-renders configs, restarts the stack, and drives the scripted `codex exec --json --skip-git-repo-check --sandbox workspace-write --full-auto` prompts. Each run starts from a clean Codex session, captures the JSONL transcripts described in `docs/e2e_clone_smoke_test.md`, and fails fast if Codex cannot see `stelae.manage_stelae`, `workspace_fs_read`, or the other required tools. Manual fallbacks continue to reuse the exact same CLI invocation; MCP wrapper releases remain optional add-ons for other experiments but are no longer part of the smoke-readiness loop.
