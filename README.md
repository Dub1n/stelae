...
- The harness now always runs with `STELAE_USE_INTENDED_CATALOG=1`; the legacy runtime override path has been retired from the smoke flow so clone validation stays focused on the intended catalog.
...
- See `docs/e2e_clone_smoke_test.md` for prerequisites plus CLI flags (`--workspace`, `--keep-workspace`, `--codex-cli`, `--codex-home`, `--force-workspace`, `--reuse-workspace`, `--cleanup-only`).
