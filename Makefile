...
ENV_FILE        ?= $(if $(STELAE_ENV_FILE),$(STELAE_ENV_FILE),$(STELAE_CONFIG_HOME)/.env)
PM2_ECOSYSTEM   ?= $(STELAE_DIR)/ecosystem.config.js
PYTHON          ?= $(if $(wildcard $(STELAE_DIR)/.venv/bin/python),$(STELAE_DIR)/.venv/bin/python,python3)
PM2             ?= pm2
...
catalog-metrics:
	@$(PYTHON) scripts/catalog_metrics.py
